...
import { useFlagshipAdminSubmissions } from "@/hooks/useFlagshipAdminSubmissions";
import type { AdminSubmission } from "@/services/adminService";
import SuccessModal from "@/components/common/SuccessModal";

const FlagshipAdminSubmissions = () => {
  const { role } = UseAuthContext();
  const certificateType =
    role?.role === "flagship_admin" ? (role.certificate_type as "Blood Donation" | "Tree Tagging") : "";

  const {
    submissions, isLoading, error, successMessage, stats,
    filters, setFilters, updateStatus, clearMessages, refetch,
  } = useFlagshipAdminSubmissions(certificateType);
  ...
  return (
    <div className="font-isans min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 container mx-auto px-4 py-8 space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
              ...
              {typeLabel} Submissions
            </h1>
            <p className="text-gray-500 mt-1">Review and approve {typeLabel.toLowerCase()} certificates.</p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>All {typeLabel} Submissions</CardTitle>
            <CardDescription>Approve or reject submissions for your certificate programme.</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-12 text-center text-muted-foreground">
                ...
                <p className="text-sm mt-1">{filters.search || filters.status ? "Try adjusting your filters." : `${typeLabel} submissions will appear here.`}</p>
              </div>
            ) : (
                      ...
                      <TableRow key={sub.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="sm" onClick={() => setSelected(sub)} title="View details"><Eye className="h-4 w-4" /></Button>
                            {sub.certificate_url && (
                              <Button variant="ghost" size="sm" onClick={() => window.open(sub.certificate_url!, "_blank")} title="Download"><Download className="h-4 w-4" /></Button>
                            )}
                            {sub.status !== "approved" && (
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {selected && (
            <div className="space-y-4">
              ...
              {selected.donation_case && (<><Separator /><div><p className="text-sm text-muted-foreground mb-1">Case Description</p><p className="text-sm">{selected.donation_case}</p></div></>)}
              {selected.tagged_tree_links && selected.tagged_tree_links.length > 0 && (
                <><Separator /><div><p className="text-sm text-muted-foreground mb-2">Tagged Tree Links</p>
                  {selected.tagged_tree_links.map((link, i) => (
                    <a key={i} href={link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-sm text-primary-600 hover:underline">
                      <ExternalLink className="h-3 w-3" />Tree {i + 1}
                    </a>
                  ))}
                </div></>
              )}
...
export default FlagshipAdminSubmissions;
