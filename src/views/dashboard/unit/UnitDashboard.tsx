...
export default function UnitDashboard({ user: propUser, onLogout }: UnitDashboardProps) {
    const { currentUser } = useMasterAuth();
    const currentUserResolved = propUser || currentUser;

    return (
        <Layout title="UNIT Dashboard" user={currentUserResolved as any} onLogout={onLogout || (() => { })}>
            <div className="space-y-6">
                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl text-white p-6">
                    <h2 className="text-2xl font-bold mb-2">UNIT Dashboard</h2>
